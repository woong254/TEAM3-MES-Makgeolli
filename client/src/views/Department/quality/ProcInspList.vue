...
// 1. 제목
const currentPageTitle = ref('공정검사 조회')
...
const fetchMatInspList = async () => {
  try {
    const { data } = await axios.get('/api/procInspList')
...
const runSearch = async () => {
  try {
    ...
    const { data } = await axios.post('/api/procInspListSearch', payload)
    const rows = Array.isArray(data) ? data : data?.data || []
    inspData.value = rows
  } catch (e) {
...
const excelColumns = [
  ...
  { key: 'procs_no', header: '공정실적번호' },
  { key: 'now_procs', header: '공정명' },
...
function exportToXlsx() {
  ...
  XLSX.utils.book_append_sheet(wb, ws, '공정검사')

  // 파일명(예: 자재입고검사_2025-10-08.xlsx)
  const today = new Date().toISOString().slice(0, 10)
  XLSX.writeFile(wb, `공정검사_${today}.xlsx`)
}
      ...
      <template #body-content>
        <div class="flex flex-wrap gap-4">
          <div class="w-1/4">
            ...
            <input type="text" :class="inputStyle" v-model="cond.insp_name" />
          </div>
          <div class="w-1/4">
