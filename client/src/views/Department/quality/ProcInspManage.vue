...
import ProcInspTargetSelectModal from './ProcInspTargetSelectModal.vue' // 검사대상(공정실적, 반제품) 조회 모달
...
import procInspSearchModal from './procInspSearchModal.vue' // 검색 모달

// 1. 페이지 타이틀
const currentPageTitle = ref('공정검사 관리')

// 2. TS 데이터타입
// 2-1. 검사대상(공정실적관리 완제품)
interface procInspTargetDT {
  procs_no: number
  prod_code: string
  prod_name: string
  prod_spec: string
...
const onInspChecked = async (row: procInspTargetDT | null) => {
  ...
  procInspTargetData.prod_name = row.prod_name
  procInspTargetData.prod_spec = row.prod_spec
  procInspTargetData.comncode_dtnm = row.comncode_dtnm
  procInspTargetData.mk_qty = row.mk_qty
  procInspTargetData.now_procs = row.now_procs
...
const onInspValue = () => {
  if (procInspQty.value > procInspTargetData.mk_qty) {
    alert('검사량은 생산량을 초과할 수 없습니다.')
    procInspQty.value = procInspTargetData.mk_qty
  }
}
// 8-2. 합격량 계산
watch([procInspQty, procInspNG], () => {
  procInspPass.value = procInspQty.value - procInspNG.value
})
// 8-3. 불량량 계산
...
watch(
  ...
  (vals) => {
    ...
    // 검사량보다 크면 바로 리셋
    if (procInspNG.value > (procInspQty.value ?? 0)) {
...
const buildPayload = () => {
  ...
  return {
    ...
    insp_qty: Number(procInspQty.value || 0),
    pass_qty: Number(procInspPass.value || 0),
    fail_qty: Number(procInspNG.value || 0),
    remark: remark.value || '',
    final_result, // 'P' | 'F'
    emp_id: inspector.value || 'EMP-20250616-0004',
    procs_no: Number(procInspTargetData.procs_no),
    results,
    ngs,
  }
}
...
function sanitizeQty() {
  const v = Number(procInspQty.value ?? 0)
  if (v < 0) {
...
const validateBeforeSubmit = (): boolean => {
  ...
  if (!procInspTargetData.prod_name) {
    alert('검사할 완제품을 선택해주세요.')
    return false
  }
  if (!procInspQty.value || procInspQty.value <= 0) {
    alert('검사량을 입력해주세요.')
    return false
  }
  if (procInspQty.value > procInspTargetData.mk_qty) {
  ...
  if (procInspPass.value < 0) {
    alert('합격량은 0보다 작을 수 없습니다.')
    return false
  }
  if (procInspNG.value > procInspQty.value) {
    alert('불량량은 검사량을 초과할 수 없습니다.')
    return false
  }
...
const submitRegister = async () => {
  try {
    ...
    const { data } = await axios.post('/api/procInsp', payload)
    if (data?.ok) {
      alert(`등록되었습니다.\n검사ID: ${data.insp_id ?? ''}`)
...
const resetForm = () => {
  ...
  // 타겟(공정실적관리)
  Object.assign(procInspTargetData, {
    procs_no: '',
    prod_code: '',
    prod_name: '',
    prod_spec: '',
...
const openSearchModal = async () => {
  try {
    const { data } = await axios.post('/api/procInspSearch', {
...
async function onPickedRow(row: modalRowDT) {
  ...
  // 2) 상세조회 호출
  await axios
    .get(`/api/procInspDetail/${row.insp_id}`)
    .then(({ data }) => {
      if (!data?.ok) {
      ...
      // 수량/비고
      procInspQty.value = Number(header.insp_qty || 0)
      procInspNG.value = Number(header.fail_qty || 0)
      procInspPass.value = Number(header.pass_qty || 0)
      remark.value = header.remark || ''

      // 공정실적(반제품) 영역
      // procInspTargetData: { procs_no, prod_code, prod_name, prod_spec, comncode_dtnm, mk_qty, procs_endtm }
      procInspTargetData.procs_no = header.procs_no || ''
      procInspTargetData.prod_code = header.prod_code || ''
      procInspTargetData.prod_name = header.prod_name || ''
      procInspTargetData.prod_spec = header.prod_spec || ''
      procInspTargetData.comncode_dtnm = header.prod_unit_name || '' // 단위 라벨
      procInspTargetData.mk_qty = Number(header.mk_qty || 0)
      procInspTargetData.procs_endtm = header.procs_endtm || ''
      procInspTargetData.now_procs = header.now_procs || ''
      ...
      for (const r of results || []) {
        if (r.insp_type === 'R') {
          ...
          // 백엔드가 상세 조회 결과에 score_desc, sens_questions 를 문자열 JSON으로 주고 있다고 가정
            ...
            <div class="flex flex-wrap justify-between gap-2">
              <div class="w-1/5 flex items-center relative">
                ...
                <input
                  ...
                  readonly
                  @click="onOpenTarget"
                  v-model="procInspTargetData.prod_code"
              ...
              <div class="w-1/5 flex items-center relative">
                ...
                <input
                  ...
                  readonly
                  @click="onOpenTarget"
                  v-model="procInspTargetData.prod_name"
                  :style="isEdit ? 'outline: none' : ''"
                />
                <button
                  type="button"
              ...
              <div class="w-1/5 flex items-center relative">
                ...
                <input
                  ...
                  readonly
                  @click="onOpenTarget"
                  v-model="procInspTargetData.procs_no"
                  :style="isEdit ? 'outline: none' : ''"
                />
                <button
                  type="button"
              ...
              <div class="w-1/5 flex items-center relative">
                <label :class="labelStyle" class="w-[120px]"> 공정명 </label>
                <input
                  type="text"
                  :class="isEdit ? inputDisabled : inputStyleClick"
                  class="w-2/3 cursor-pointer hover:bg-gray-100 duration-300"
                  readonly
                  @click="onOpenTarget"
                  v-model="procInspTargetData.now_procs"
                  :style="isEdit ? 'outline: none' : ''"
                />
                <button
                  type="button"
              ...
              <div class="w-1/5 flex items-center relative">
                ...
                <input
                  ...
                  class="w-2/3 cursor-pointer hover:bg-gray-100 duration-300"
                  readonly
                  v-model="procInspTargetData.prod_spec"
                  @click="onOpenTarget"
                  :style="isEdit ? 'outline: none' : ''"
                />
                <button
              ...
              <div class="w-1/5 flex items-center relative">
                ...
                <input
                  ...
                  readonly
                  @click="onOpenTarget"
                  v-model="procInspTargetData.comncode_dtnm"
                  :style="isEdit ? 'outline: none' : ''"
                />
                <button
                  type="button"
              ...
              <div class="w-1/5 flex items-center relative">
                ...
                <input
                  ...
                  readonly
                  @click="onOpenTarget"
                  v-model="procInspTargetData.mk_qty"
          ...
          <div class="rounded-lg border border-gray-200 p-4 shadow-sm mb-2">
            ...
            <div class="flex flex-wrap mb-2">
              <div class="w-1/4 flex items-center">
                ...
                <div class="text-sm w-[100px] ml-2">
                  {{ procInspTargetData.comncode_dtnm || '단위' }}
                </div>
              </div>
              <div class="w-1/4 flex items-center">
            ...
            <div class="flex flex-wrap mb-2">
              ...
              <div v-if="ng.length == 0">
                <div v-if="!procInspTargetData.prod_name">
                  <span class="text-sm" style="color: #999">불량 항목이 비어 있습니다.</span>
                </div>
                <div v-else>
                  <span class="text-sm" style="color: #999"
                    >선택한 자재 '{{ procInspTargetData.prod_name }}'에 연결된 불량 항목이 없습니다.
                    불량 기준을 등록해주세요.
                  </span>
                </div>
              </div>
